...
import { Bell, CheckCircle2, ChevronLeft, ChevronRight, CreditCard, FileText, Image as ImageIcon, Languages, LayoutDashboard, LogOut, Mic, Moon, Paperclip, Plus, RefreshCcw, Search, Send, Sun, Ticket, Trash2, User, Users, X, type LucideIcon } from "lucide-react"
import { type AdminUser, type FeedItem, type FeedType, type Profile, type Role, type SupportTicket, ensureAdminProfile, logout, redirectToAuth, request } from "@/lib/api"
...
function Overview({ service, setService }: { service: Service; setService: (v: Service)=>void }) {
  ...
  const distributionTotal = Object.values(data?.subscription_distribution || {}).reduce((a,b)=>a+b,0)
  return <section className="page page-enter"><ServiceSwitch value={service} onChange={setService}/><div className="metric-grid"><Metric label="Пользователи" value={data?.users_total}/><Metric label="Открытые тикеты" value={data?.open_tickets}/><Metric label="Ждут пользователя" value={data?.answered_tickets}/><Metric label="События ленты" value={data?.feed_items}/><Metric label="Подписки" value={data?.active_subscriptions}/></div><div className="charts-grid"><div className="data-block"><h2>Подписки</h2><div className="donut-row">{["free","plus","pro"].map((plan)=><div key={plan} className="donut-line"><span>{planLabel(plan)}</span><b>{data?.subscription_distribution?.[plan] || 0}</b><i style={{width: `${distributionTotal ? ((data?.subscription_distribution?.[plan] || 0) / distributionTotal) * 100 : 0}%`}}/></div>)}</div></div><div className="data-block"><h2>Новые пользователи</h2><MiniBars rows={data?.new_users || []}/></div><div className="data-block"><h2>API-запросы</h2><MiniBars rows={data?.api_requests || []}/></div></div></section>
}
...
function FeedPanel({ service }: { service: Service }) {
  const [items,setItems]=useState<FeedItem[]>([]); const [type,setType]=useState<FeedType>("reminder"); const [target,setTarget]=useState(""); const [dateTime,setDateTime]=useState(""); const [title,setTitle]=useState(""); const [body,setBody]=useState(""); const [points,setPoints]=useState([""])
...
function TicketPanel({ ticketId }: { ticketId?: string }) {
  const [ticket,setTicket]=useState<SupportTicket|null>(null); const [message,setMessage]=useState(""); const [files,setFiles]=useState<File[]>([]); const [sending,setSending]=useState(false)
  ...
  return <section className="chat page-enter"><div className="chat-head"><div><Link href="/tickets">← Тикеты</Link><h2>{ticket.subject}</h2><p>#{ticket.public_number} · {displayTicketUser(ticket)} · {ticket.service || "planner"}</p></div><TicketStatusBadge status={ticket.status}/>{!closed&&<button className="secondary" onClick={()=>void close()}><CheckCircle2 size={16}/>Закрыть</button>}</div><div className="messages">{ticket.messages?.map(m=><div className={`msg ${m.author_role}`} key={m.id}><b>{m.author_role}</b><p>{m.body}</p>{m.attachments?.length ? <div className="attachments">{m.attachments.map(a=><span key={a.stored_name}>{a.original_name}</span>)}</div>:null}</div>)}</div><div className={closed ? "composer disabled" : "composer"}><label><Paperclip size={18}/><input type="file" multiple hidden onChange={e=>setFiles(Array.from(e.target.files||[]))}/></label><button><ImageIcon size={18}/></button><button><FileText size={18}/></button><input disabled={closed} value={closed ? "Тикет закрыт" : message} onChange={e=>setMessage(e.target.value)} onKeyDown={e=>{if(e.key==="Enter")void send()}}/><button><Mic size={18}/></button><button className="primary" disabled={closed||sending} onClick={()=>void send()}><Send size={17}/></button></div></section>
}
...
function MiniBars({rows}:{rows:Array<{date:string;count:number}>}){const max=Math.max(1,...rows.map(r=>r.count));return <div className="mini-bars">{rows.length?rows.map(r=><i key={r.date} title={`${r.date}: ${r.count}`} style={{height:`${Math.max(8,(r.count/max)*100)}%`}}/>):<span>Нет данных</span>}</div>}
